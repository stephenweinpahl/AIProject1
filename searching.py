...
def evaluate(num):
    ...
    plt.suptitle("Cost vs Performance over 50 iterations, A* only")
    plt.title("red = Alg2, green = alg3, blue = alg4")
    plt.xlabel("Cost")
    plt.ylabel("Time (s)")
    plt.scatter(algo2Cost, algo2Perf, color='r')
    x = np.array(algo2Cost)
    y = np.array((algo2Perf))
    m, b = np.polyfit(x, y, 1)
    plt.plot(x, m * x + b, 'r')
    plt.scatter(algo3Cost, algo3Perf, color='g')
    x = np.array(algo3Cost)
    y = np.array((algo3Perf))
    m, b = np.polyfit(x, y, 1)
    plt.plot(x, m * x + b, 'g')
    plt.scatter(algo4Cost, algo4Perf, color='b')
    x = np.array(algo4Cost)
    y = np.array((algo4Perf))
    m, b = np.polyfit(x, y, 1)
    plt.plot(x, m * x + b, 'b')
